...
export default function MoneyRequestActions({
    ...
    return (
        <div className="mt-2">
            ...
            <div className="relative justify-center text-lg font-medium mb-1 flex items-center gap-2">{moneyRequest.by_me ? "You have " : `${moneyRequest?.from?.name} has `} requested
                <div className={cn("font-bold", moneyRequest?.by_me ? 'text-green-500' : 'text-red-500')}>{(moneyRequest.by_me ? '+' : '-') + moneyRequest?.amount} BDT </div>
            ...
            <div className="flex flex-col gap-2">
                <div className="grid grid-cols-2 items-center gap-2 justify-center">
                    ...
                    {
                        moneyRequest.accepted_at && moneyRequest.by_me && <Button type="button" onClick={requestRelease} loading={processing} disabled={moneyRequest.release_requested_at != null} className="col-span-2 disabled:cursor-not-allowed text-black bg-[#D8BBFF]">{
                            moneyRequest.release_requested_at ? 'Release Requested' : 'Request Release'
                        }</Button>
                    }
                    {
                        moneyRequest.release_requested_at && <Button type="button" variant={moneyRequest.released_at ? "success" : "warning"} onClick={release} loading={processing} disabled={moneyRequest.released_at != null} className={cn("col-span-2", (moneyRequest.by_me || moneyRequest.released_at != null) && 'cursor-not-allowed')}>{
