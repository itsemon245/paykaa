...
export default function DashboardLayout({ children, animate }: { children?: JSX.Element | JSX.Element[], animate?: boolean }) {
    ...
    const LinkItem = ({
        ...
        return <Link href={item.url!}>
            <button className={cn("text-center px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg font-semibold max-sm:text-sm", item.active ? 'bg-[var(--green-500)]  text-white' : 'text-gray-700')}>
                {item.label}
            </button>
        </Link>
    }
